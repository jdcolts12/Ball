-- Fix daily leaderboard to show ALL players including those without profiles
-- Uses LEFT JOIN so users without profiles still show up
...
comment on function public.get_daily_leaderboard(int) is 'Returns daily leaderboard: ALL users who played today (UTC date), including those without profiles. Ordered by % correct (best to worst). No limit by default.';
