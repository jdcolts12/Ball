-- Remove limit entirely from daily leaderboard - show ALL players who played today
...
comment on function public.get_daily_leaderboard(int) is 'Returns daily leaderboard: ALL users who played today (UTC date), ordered by % correct (best to worst). No limit by default - shows everyone.';
